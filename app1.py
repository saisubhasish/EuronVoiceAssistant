...
#             while True:
#                 # Record Audio
#                 record_response = requests.post("http://localhost:8000/record-audio/")
#                 if record_response.status_code == 200:
#                     st.info("Audio recorded successfully")
#                 else:
#                     st.error("Failed to record audio")
#                     break  # Exit loop if recording fails

#                 time.sleep(1)  # Add delay before checking file existence

#                 if not os.path.exists("temp_audio_chunk.wav"):
#                     st.error("Audio file not found")
#                     break

#                 # Transcribe Audio
#                 try:
#                     with open("temp_audio_chunk.wav", "rb") as audio_file:
...
#                             json={"user_question": text}
#                         )
#                         if response_response.status_code == 200:
#                             response_llm = response_response.json()["response"]
#                             st.markdown(
#                                 f'<div style="background-color: #f0f0f0; padding: 10px; border-radius: 5px;">AI Assistant 🤖: {response_llm}</div>',
#                                 unsafe_allow_html=True
#                             )
...
#                     st.error("Failed to open audio file")
#                     break
